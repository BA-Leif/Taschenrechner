...
namespace Taschenrechner
{
    public class Parser_geht
    {
        ...
        public static double Start(String input)
        {
            ...
            {
                calculationSteps++;
                //Ersetzen des zuvor berechneten Termes durch sienen Wert. Sollte der Term vorher in Klammer eingefasst sein, so werden diese auch ersetzt.
                ...
                Console.WriteLine("Ergebniss vom priorisierten Teil     " + subInputAnswer);
                Console.WriteLine("Einsetzen von " + subInputAnswer + " anstatt von " + subInputWithoutBrackets + " in " + input);

                Console.ReadLine();
            }
            while (input != subInputWithoutBrackets);
            //Initialisieren und übergeben der Ausgabeveriable 
            double output = Double.Parse(subInputAnswer);
            return output;
        }
        ...
        static string FindBrackets(string input)
        {
            ...
            int lastLeftBracket = ((input.Length - 1) + 1);
            //Suchen nach der letzten eröffnenden Klammer und der ersten schliessenden Klammer
            //
            //  PROBLEM : (...)+(...)
            //
        ...
        static string PrepareForCalculation(string sub)
        {
            ...
            if (operationPosition != 0)
            {
                ...
                {
                    Console.WriteLine("Ungültige Eingabe im Subterm " + sub + ".");
                }
