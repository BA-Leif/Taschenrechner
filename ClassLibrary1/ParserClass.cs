    ...
    public class ParserClass
    {
        ...
        public static double Start(String input)
        {
            ...
            do
            {
                ...
                {
                    input = replaceFirst(input, subInputWithoutBrackets, subInputAnswer);
                    //input = input.Replace(subInputWithoutBrackets, subInputAnswer);
                    input = input.Replace("()", "");
                }
        ...
        static string OrderOfOperation(String sub)
        {
            ...
            if (sub.Contains(")"))
            {
                ...
                sub = sub.Substring(1, (sub.Length - 1) - 1);
            }
            ...
            if (operationPosition != 0)
            {
                for (int i = (operationPosition - 1); i >= 0; i--)
                {
                    ...
                    if ((!(Char.IsNumber(sub[i]) || sub[i] == ',') || i == 0))
                    {
                        ...
                        if (i == 0)
                        {
                            XString = sub.Substring(i, operationPosition - i);
                            break;
                        }
                        ...
                        {
                            XString = sub.Substring(i + 1, operationPosition - (i + 1));
                            break;
                        }
                    ...
                    for (int i = (operationPosition + 1); i <= sub.Length - 1; i++)
                    {
                        ...
                        if (!(i == (operationPosition + 1) && !(Char.IsNumber(sub[i]))))
                        {
                            ...
                            if (!(Char.IsNumber(sub[i]) || sub[i] == ',') || i == (sub.Length - 1))
                            {
                                ...
                                if (i == (sub.Length - 1))
                                {
                                    YString = sub.Substring((operationPosition + 1), (sub.Length - (operationPosition + 1)));
                                    break;
                                }
                                //Suche ist auf eine Rechenoperation gestoßen (kann kein Vorzeichen sein)
                                else
                                {
                                    YString = sub.Substring((operationPosition + 1), (i - (operationPosition + 1)));
                                    break;
                                }
        ...
        static string PrepareForCalculation(string sub)
        {
            ...
            if (operationPosition != 0)
            {
                ...
                output = CalculationClass.ChooseOperation(X, Y, operationIndex).ToString();
            }
        ...
        {
            int oldSStrLength = oldSubterm.Length;;
            string ReplAllStr = wholeTerm;

            for (int i = 0; i < ReplAllStr.Length-(oldSStrLength); i++)
            {
                
                if (ReplAllStr.Substring(i, oldSStrLength).Contains(oldSubterm))
                {
                    ReplAllStr = ReplAllStr.Remove(i, oldSStrLength);
